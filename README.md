...
- Google login/logout
- `Backlog` and `Started` filtering via the header filter's `Status` dropdown, with `Done` and `Deleted` available from the `...` menu
- Menu-based `Review` view and a daily/weekly closing summary
- Toggleable Kanban view with persistent optional `clarify`, `Backlog`, and `Started` columns
...
On mobile capture and mobile editing, the derived `When` and `Prio` values are shown so changes to the criteria stay visible without making those values manually selectable.
...
Supabase stores the managed tag catalog in `user_settings.available_tags`, selected tag tabs in `user_settings.selected_tag_tabs`, browser overview mode in `user_settings.browser_compact_view`, tooltip preference, separate browser/phone dark-mode preferences, configurable card badge columns including browser Kanban, the separate browser/phone default list/Kanban view modes, active Kanban columns, separate browser/phone edit-section defaults, and the custom three-row tab layout. Active tab, tag scope, search text, filters, and column sorts are intentionally session-only and are not restored on a fresh app start. Apply the schema update before relying on synced settings across phone and browser:
...
The desktop overview and the header filter popup can filter the visible task list by the table columns, including ID, When, Prio, Tag, task text, description, subtasks, predecessors, successors, status, start date, due date, done date, and deleted date where those columns are visible. The `Status` filter remains an editable dropdown for `All`, `Backlog`, and `Started`; `Done` and `Deleted` stay in the scoped `...` menu views. Choosing status or due filters activates the matching visible tabs where such tabs exist. The `ID` filter combines free text input with a dropdown of existing task IDs, so tasks can be narrowed directly by codes like `T-016`.
...
Status values are `Backlog`, `Started`, `Done`, and `Deleted`; `Backlog` and `Started` are reached through the header filter's `Status` dropdown, while `Done` and `Deleted` are opened from the `...` options menu. `Deleted` is also available in the status dropdown: choosing it sets the soft-delete date, while choosing `Backlog`, `Started`, or `Done` from a deleted task restores it into that status. When a task is marked done, the app stores the done date automatically and stays in the previous view instead of jumping to `Done`. Done tasks are shown in the `Done` menu view with the `Done on` column; compact cards show `done on [date]`. The `Done` view sorts by newest done date first by default, and the column still supports text filtering and manual ascending/descending sorting.
...
The `...` menu entry `Close-out` shows a compact daily/weekly review: done today, done in the last 7 days, started, still open, and overdue, plus direct links to current Review tasks.
...
In browser and phone capture/edit mode, editable values and derived values expose tooltips with the current selection and a short explanation while `Show tooltips` is enabled in the `...` options menu. Derived `When` and `Prio` tooltips include their selected criteria.
...
- Task card badges use the fixed order `When`, `Prio`, `Status`, `Tag`, `Created`, `Start`, `Due`; non-applicable badge slots use invisible placeholders so card layouts stay stable.
