...
- Add, edit, complete, delete, search, import, and export tasks
- Online task persistence via Supabase
- Google login/logout
- `Done` and `Deleted` each have their own top-level tab next to `All`; `Backlog` and `Doing` are not separately filterable and both show together in `All`
...
- Mobile task cards with compact action icons for descriptions, deletion, and completion
...
The `...` options menu includes separate persistent `Layout Browser` and `Layout phone` selectors for `Normal` or `Dark Mode`, a persistent `Show tooltips` checkbox, a `Default task details` Minimum/Maximum selector. The four edit sections (`Parameter`, `Description`, `Subtasks`, `Comments`) always default to expanded on both browser and phone; this is not configurable. Task cards default to `Minimum`, meaning parameter badges and card content stay hidden until a task's ID is clicked. `Maximum` shows parameter badges by default; cards with a description, subtasks, or comments show their content panels directly below the badges, with no click needed. The header task-detail icon temporarily switches the current List/Kanban view between Minimum and Maximum without changing the saved default; its tooltip starts with `View:`. The menu controls themselves have short tooltips explaining what each option does. `Show tooltips` defaults to enabled; when active, capture and edit fields on browser and phone expose tooltips with the current value and a short explanation. The `Prio` tooltip also states that `Prio`/priority answers `How important?`, then shows the selected criteria that produced the derived value and the default sort order. Dark Mode, task-card detail default, browser Kanban badge column count, separate browser/phone default list/Kanban view, tooltips, Browser compact, tab layout, and tag catalog sync through `user_settings` when the schema columns exist, with local browser storage as fallback. Current working selections such as active tab, tag scope, search text, the temporary List/Kanban toggle, and the temporary task-detail toggle stay session-only while the app is open; a fresh app session starts from the configured option defaults.
...
- Icon order is always: edit, delete, completion. Adjacent action icons use the same visual size throughout the UI and are aligned to the right side of the card header, including edit mode. The old Details icon is replaced by the card's content panels: in Maximum detail mode, a task's description/subtasks/comments panels show directly below the badges with no toggle; in Minimum detail mode, clicking the task ID reveals just that card's panels below the badges, only when the task has a description, subtasks, or comments; clicking it again, or clicking another task ID, closes it, without changing the global view mode. An upcoming task shows a small red `!` marker directly left of the task ID in overview and edit mode. Its tooltip lists the concrete reasons such as `Start reached`, `due today`, or subtask reminders, including on touch devices. If a task title wraps to multiple lines, the `!` marker and task ID stay aligned with the first title line. In edit mode the task ID is a plain, non-clickable label. Editing starts from the pencil icon. In mobile edit mode, the edited card header uses the same width as the colored edit-section dividers; the optional `!` marker and task number stay left, and action icons are anchored to the right edge of the card.
...
- A direct task URL such as `https://task-001.pixelina.me/?task-id=T-123` opens that task in the normal Maximum detail view for the matching user's task list after login; use the pencil icon to edit it.
...
The full description is shown in read mode inside the card's expanded
detail panel, with no length truncation.
...
Supabase stores the managed tag catalog in `user_settings.available_tags`, selected tag tabs in `user_settings.selected_tag_tabs`, tooltip preference, separate browser/phone dark-mode preferences, the configurable browser Kanban badge-column count, the separate browser/phone default list/Kanban view modes, the deleted-task retention period, and the custom three-row tab layout. Active tab, tag scope, and search text are intentionally session-only and are not restored on a fresh app start. Apply the schema update before relying on synced settings across phone and browser:
...
A task cannot be marked as done while it still has open subtasks. Complete subtasks first, then mark the task as done.

The info line below the tabs always shows the current working selection: view, scope, list/Kanban mode, and search state. The neutral active-task view is shown as `All active`, so tag tabs remain consistent as a scope such as `#ETH`. This row is hidden in single-task edit mode.
...
Browser and phone now each open on the `All` tab on every fresh start. Task capture is opened with the plus icon. Tab selection, tag scope, and search text stay in memory only for the current app session; return/navigation flows can still use that in-session state, but closing and reopening the app starts from the configured option defaults. Task cards default to Minimum detail mode, where badges and card content stay hidden until a task's ID is clicked. Maximum detail mode shows the badge grid, and tasks with description, subtasks, or comments show that content directly below the badges, with no click needed.
While editing tasks in the browser, short controls such as dropdowns and date fields stay narrow; the task title, description, subtasks, and comments share the same controlled editing width so the card background does not stretch wider than the fields. In the browser parameter editor, Tag and Status form one row, followed by Start date and Due on the next row. Small dropdowns do not show a redundant close X; single-choice dropdowns such as Tag close after selection. On phones, the same order is kept but each parameter uses its own row. Comment and subtask text inputs use the same full editor width and text size as the description field, with content-based height.
Status values are `Backlog`, `Doing`, `Done`, and `Deleted`; `Backlog` and `Doing` are not separately viewable and both show together in `All`, `Done` has its own top-level tab next to `All` (and is also available from the `...` options menu), while `Deleted` is opened from the `...` options menu. The edit-mode `Status` dropdown can set any task directly to `Backlog`, `Doing`, `Done`, or `Deleted`: choosing `Deleted` sets the soft-delete date, while choosing `Backlog`, `Doing`, or `Done` from a deleted task restores it into that status. When a task is marked done, the app stores the done date automatically and stays in the previous view instead of jumping to `Done`. Done tasks are shown in the `Done` menu view with the `Done on` column; compact cards show `done on [date]`. The `Done` view sorts by newest done date first by default.
...
Task edit mode is divided into four clearly separated, individually collapsible sections: `Parameter`, `Description`, `Subtasks`, and `Comments`. While `Parameter` is collapsed, it keeps showing the same compact overview values. Tapping a value opens only its tooltip; tapping the `Parameter` label or arrow expands the editable controls. Empty description, comment, and subtask areas show their prompt directly inside the field: `Write a new description`, `Write a new comment`, or `Write a new subtask`. Description, subtasks, and comments are edited inline with the same local save/discard pattern. No separate empty-state text is rendered below the fields. All sections always default to expanded on both browser and phone; there is no setting to change this. Card detail mode controls overview cards separately: `Minimum` hides parameter badges and card content until a task's ID is clicked; `Maximum` shows the parameter badges by default. In Maximum mode, cards with a description, subtasks, or comments show their labeled panels (`Description`, `Subtasks`, `Comments`) directly below the badges, with no click needed. In Minimum mode, clicking a card's task ID reveals that card's panels (also including a `Parameter` panel, since badges aren't otherwise visible there); clicking another task ID closes the previously opened one before opening the new one, and clicking the same task ID again closes it. The header task-detail icon switches the current view immediately between Minimum and Maximum, while Options define only the default for fresh sessions. Compact browser cards and phone editing use section headings, spacing, and colored divider lines. In the options menu, browser Kanban cards' badge columns can be set as `Default (3)` or any value from 1 to 8; this setting applies only to browser Kanban view. Browser overview and edit cards always use 3 badge columns, matching phone. Phone list, edit, and Kanban cards keep the mobile card width and fixed 3-column mobile badge layout. Card rows are padded with empty `-` slots so the final row stays full and cards keep a consistent grid width.

In Dark Mode, dialogs, dropdowns, picker popups, parameter controls including the Tag selector, the mobile Prio row, and the derived Prio display while capturing a new task on browser or phone use dark surfaces with light text. `Prio` badges use the same semantic colors in browser and phone views, regardless of Normal or Dark Mode.
...
Recent schema requirements include `tasks.tags` for task tag assignments, `user_settings.available_tags` for the synced tag catalog, `user_settings.selected_tag_tabs` for synced tag-tab settings, `user_settings.tab_layout` for the synced three-row tab arrangement, `user_settings.card_badge_columns` for the browser-Kanban badge-column count, `user_settings.default_view_mode` for the synced list/Kanban default, `user_settings.deleted_retention_days` for the synced deleted-task retention period, and `allowed_users` for app-managed Google access.
...
- In Minimum detail mode, compact cards hide parameter badges and card content by default in List and Kanban until a task's ID is clicked. Maximum detail mode shows the badges, and tasks with a description, subtasks, or comments show that content directly, with no click needed.
