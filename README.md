...
- Google login/logout
- `Backlog` and `Doing` filtering via the header filter's `Status` dropdown, with `Done` and `Deleted` available from the `...` menu
- Menu-based `Review` view and a daily/weekly closing summary
- Toggleable Kanban view with persistent optional `clarify`, `Backlog`, and `Doing` columns
...
separate `Doing` status and private work is handled through tags such as `Private`.
...
The `...` menu entry `Close-out` shows a compact daily/weekly review: done today, done in the last 7 days, doing, still backlog, and overdue, plus direct links to current Review tasks.
