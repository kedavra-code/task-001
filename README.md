...
- Google login/logout
- `Backlog` and `Doing` filtering via the header filter's `Status` dropdown; `Done` and `Deleted` each have their own top-level tab next to `All` and are also available from the `...` menu
